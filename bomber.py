...
def bann_text():
    ...
    logo = """
       ...
       🔴 🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴
       🔴  LETS DO IT! 
       🔴  WELCOME YOU LEGEND! ITS TIME TO TAKE REVENGE!
       🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴
                                         """
    if ASCII_MODE:
        logo = ""
...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/AlexBieber/RevengeBomber/archive/dev.zip"
        dir_name = "RevengeBomber-dev"
    else:
        zip_url = "https://github.com/AlexBieber/RevengeBomber/archive/master.zip"
        dir_name = "RevengeBomber-master"
    ...
    if success:
        mesgdcrt.SuccessMessage(
            "RevengeBomber was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update RevengeBomber.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/AlexBieber/RevengeBomber.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/AlexBieber/RevengeBomber.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/AlexBieber/RevengeBomber/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("RevengeBomber is up-to-date")
        mesgdcrt.GeneralMessage("Starting RevengeBomber")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/AlexBieber/RevengeBomber/raw/dev/.notify"
        else:
            url = "https://github.com/AlexBieber/RevengeBomber/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("RevengeBomber was created by AlexBieber")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    print("\n")
    mesgdcrt.SuccessMessage("Revenge completed!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("RevengeBomber will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['AlexBieber', 't0xic0der', 'scpketer', 'Stefan']
...
parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by AlexBieber !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start RevengeBomber with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start RevengeBomber with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start RevengeBomber with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update RevengeBomber")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current RevengeBomber contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current RevengeBomber version")
